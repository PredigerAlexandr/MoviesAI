...
public class GetMovieJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        try
        {
            ...
            {
                string url =
                    $"https://api.kinopoisk.dev/v1.4/movie?page={page}&limit=200&selectFields=createdAt&selectFields=id&notNullFields=createdAt&otNullFields=year" +
                    $"&selectFields=name&selectFields=genres&selectFields=countries&selectFields=year&selectFields=persons&sortField=createdAt&sortType=1&rating.kp=8-10&notNullFields=poster.url&selectFields=poster";
                // Отправка GET-запроса
                ...
                foreach (var newMovie in newMovies)
                {
                    var movieEntity = new MovieEntity()
                    {
                        ...
                        Actors = newMovie.Persons.Select(x => x.Name).ToList(),
